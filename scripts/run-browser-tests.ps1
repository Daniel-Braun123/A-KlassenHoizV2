...
if (-not $env:PLAYWRIGHT_WEB_SERVER_COMMAND) {
    & (Join-Path $nodeRoot 'node.exe') (Join-Path $repositoryRoot 'node_modules\next\dist\bin\next') build
...
function Invoke-BrowserProject([string]$ProjectName) {
    ...
    & (Join-Path $nodeRoot 'node.exe') $playwright @arguments
...
foreach ($projectName in $projects) {
    if ($Suite -ne 'pwa') {
        & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $repositoryRoot 'scripts\database.ps1') reset
