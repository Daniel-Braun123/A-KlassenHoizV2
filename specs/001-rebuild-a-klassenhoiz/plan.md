...
| Supabase Connector/MCP | Preview und Production identifizieren; Schema, RLS, Views, Auth, Storage, Zähler und Advisors ohne Secret-Ausgabe verifizieren | Used / Preview and Production verified |
| Supabase CLI 2.109.1 | Isolierte Preview-/Production-Workdirs verknüpfen, exakt 51 V2-Migrationen migrations-only anwenden, Konfiguration setzen, Dry-Run und Smoke-Bereinigung ausführen | Used / Production up to date |
...
| GitHub CLI und `origin` | Repository, `main`, Pushstatus und Commit-Historie verifizieren und den finalen Stand veröffentlichen | Used / `main` canonical and pushed |
| Vercel CLI 56.0.0 | Projekt/Actor prüfen, Alt-Git-Link trennen, Production-Variablen bytegenau setzen, V2 deployen, Alias und Live-Smoke verifizieren | Used / Production READY |

Capability-Einfluss: Die Supabase-Fähigkeiten führten zur `api`/`app`/`private`-Grenze, DB-Zeit/RPC-Transaktionen und vollständigen Grant-/RLS-Matrix. React-, Design- und Playwright-Fähigkeiten führten zu Server-first Rendering, kleinen Client-Grenzen, tokenisierten Zuständen, 44-px-Zielen sowie Cross-Browser-, A11y- und PWA-Tests. Der Implementierungslauf verwendete Node 24.18.0/npm 11.18.0, 51 neue Migrationen und vollständige lokale CI. `systematic-debugging` isolierte im Rollout native PowerShell-Stderr-Effekte, ein altes .NET-API-Problem, einen synthetischen Fixture-Rest, ein unsichtbares UTF-8-BOM in Vercel-Werten und ein zu breites Ignore-Muster. Supabase ist migriert und bereinigt; Vercel Production ist `READY`; GitHub `main` ist kanonisch. Manuelle Screenreader-/Gerätetests, der Fünf-Personen-Test und zwei LCP-Budgets bleiben transparent dokumentierte Product-Owner-V1-Ausnahmen.
