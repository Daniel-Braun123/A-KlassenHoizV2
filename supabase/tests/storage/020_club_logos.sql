...
select ok((select with_check not like '%metadata%' from pg_policies where policyname = 'club_logos_admin_insert'), 'insert policy accepts the Storage metadata lifecycle');
